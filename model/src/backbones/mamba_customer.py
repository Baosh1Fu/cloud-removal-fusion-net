...
class ConvMambaBlock(nn.Module):
    def __init__(self, d_model, d_state=16, d_conv=3, expand=2, conv_mode="deepwise"):
        super().__init__()
        ...
        self.local = (
            nn.Sequential(
                ...
                nn.SiLU(),
                LightweightModel(d_model, self.d_inner)
    ...
    def forward(self, x):
        if x.ndim == 4:
            ...
            x_in = x
            local = self.local(x)
            local = rearrange(local, "b d h w -> b d (h w)")
        ...
        dt = self.dt_proj(dt).view(B, L, self.d_inner).permute(0, 2, 1)
        B_ = B_.view(B, L, self.d_state).permute(0, 2, 1)
        C_ = C_.view(B, L, self.d_state).permute(0, 2, 1)

        A = -torch.exp(self.A_log).to(x.device)
        y = selective_scan_fn(x, dt, A, B_, C_, self.D, z=z, delta_softplus=True)
